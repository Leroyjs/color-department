...
import {formatPhoneNumber, getDimensions, postOrder, withFormProvider} from 'utils'
import {
    H3,
    ButtonRectangle,
    DropDownField,
...
export const ShopForm = withFormProvider(
    ({cost, description, dimensions, title}) => {
        ...
        const onSubmit = (data) => {
            postOrder({...data, name: title, quantity: String(data.quantity), "total_cost": getTotal(data)})
                .then(() => {
                    setOpen(true)
                })
                .catch((error) => {
